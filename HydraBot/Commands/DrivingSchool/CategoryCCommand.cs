...
namespace HydraBot.Commands.DrivingSchool
{
    public class CategoryCCommand : INucleusCommand
    {
        ...
        public void Execute(Message msg, IMessageSenderService sender, IBot bot)
        {
            ...
            kb.AddButton("1", "catC", new List<string>() { $"{q + 1}", "1", countTrueResponse.ToString() });
            kb.AddButton("2", "catC", new List<string>() { $"{q + 1}", "2", countTrueResponse.ToString() });
            kb.AddLine();
            kb.AddButton("3", "catC", new List<string>() { $"{q + 1}", "3", countTrueResponse.ToString() });
            kb.AddButton("4", "catC", new List<string>() { $"{q + 1}", "4", countTrueResponse.ToString() });
            kb.SetOneTime();
